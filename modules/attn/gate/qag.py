...
class SelfAttn(SelfAttnBase):
	...
	def forward(self, iQ, mask=None, states=None, **kwargs):
		...
		out = self.outer(out.mul_(self.gate(torch.cat((out, _real_iQ,), dim=-1))).view(bsize, nquery, self.hsize))
...
class CrossAttn(CrossAttnBase):
	...
	def forward(self, iQ, iK, mask=None, **kwargs):
		...
		return self.outer(out.mul_(self.gate(torch.cat((out, _real_iQ,), dim=-1))).view(bsize, nquery, self.hsize))
