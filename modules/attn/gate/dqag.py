...
class SelfAttn(SelfAttnBase):
	...
	def __init__(self, isize, hsize, osize, num_head=8, dropout=0.0, enable_bias=enable_prev_ln_bias_default, enable_proj_bias=enable_proj_bias_default, **kwargs):
		...
		self.gate_transa = GroupLinear(self.hsize, self.hsize, num_head, bias=enable_bias, trans_input=False, shuffle=False, flatten_output=False)
	...
	def forward(self, iQ, mask=None, states=None, **kwargs):
		...
		out = self.outer((self.gate_act(self.gate_transa(out) + real_iQG) * out).view(bsize, nquery, self.hsize))
...
class CrossAttn(CrossAttnBase):
	...
	def forward(self, iQ, iK, mask=None, **kwargs):
		...
		return self.outer((self.gate_act(self.gate_transa(out) + real_iQG) * out).view(bsize, nquery, self.hsize))
