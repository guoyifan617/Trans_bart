...
class SelfAttn(SelfAttnBase):
	...
	def __init__(self, isize, hsize, osize, num_head=8, dropout=0.0, enable_bias=enable_prev_ln_bias_default, **kwargs):
		...
		self.gate = nn.Sequential(GroupLinear(self.hsize, self.hsize, num_head, bias=enable_bias, trans_input=False, shuffle=False, flatten_output=False), nn.LayerNorm((num_head, self.attn_dim,), eps=ieps_ln_default, elementwise_affine=enable_ln_parameters), nn.Sigmoid())
	...
	def forward(self, iQ, mask=None, states=None, **kwargs):
		...
		out = self.outer((out * self.gate(out)).view(bsize, nquery, self.hsize))
...
class CrossAttn(CrossAttnBase):
	...
	def forward(self, iQ, iK, mask=None, **kwargs):
		...
		return self.outer((out * self.gate(out)).view(bsize, nquery, self.hsize))
