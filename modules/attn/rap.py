...
class NoGradLinearCoreFunction(Function):
	...
	def forward(ctx, inputs, weight, bias=None):
		...
		if bias is None:
			return torch.mm(inputs, weight.t())
		else:
	...
	def backward(ctx, grad_outputs):
		...
		if grad_outputs is not None and ctx.needs_input_grad[0]:
			_weight = ctx.saved_tensors[0]
			return torch.mm(grad_outputs, _weight), None, None
		else:
			return None, None, None

NoGradLinearCoreFunc = NoGradLinearCoreFunction.apply
...
class NoGradLinear(nn.Module):
	...
	def forward(self, inputs, **kwargs):
		...
		_size = list(inputs.size())
		out = NoGradLinearCoreFunc(inputs.view(-1, _size[-1]), self.weight, self.bias)
		_size[-1] = -1
		return out.view(_size)
