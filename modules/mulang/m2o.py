...
class PositionwiseFF(PositionwiseFFBase):
	...
	def forward(self, x, weight=None, taskid=None, **kwargs):
		...
		if weight is not None:
			...
			del _osize[-2]
			out = bmv(out.transpose(-1, -2).contiguous().view(out.size(0), -1, self.ngroup), weight).view(_osize)
...
class SelfAttn(SelfAttnBase):
	...
	def forward(self, iQ, mask=None, states=None, weight=None, **kwargs):
		...
		if weight is not None:
			_osize = list(out.size())
			del _osize[-2]
			out = bmv(out.transpose(-1, -2).contiguous().view(bsize, -1, ngroup), weight).view(_osize)
