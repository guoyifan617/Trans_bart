...
class PositionwiseFF(PositionwiseFFBase):
	...
	def __init__(self, isize, ngroup, hsize=None, dropout=0.0, ntask=None, custom_act=use_adv_act_default, enable_bias=enable_prev_ln_bias_default, **kwargs):
		...
		self.net = nn.Sequential(Linear(isize, _hsize), Custom_Act() if custom_act else nn.ReLU(inplace=True), Dropout(dropout, inplace=inplace_after_Custom_Act), GroupLinear(_hsize, isize * ngroup, ngroup, bias=enable_bias, shuffle=False, trans_input=True, flatten_output=False), Dropout(dropout, inplace=True)) if dropout > 0.0 else nn.Sequential(Linear(isize, _hsize), Custom_Act() if custom_act else nn.ReLU(inplace=True), GroupLinear(_hsize, isize * ngroup, ngroup, bias=enable_bias, shuffle=False, trans_input=True, flatten_output=False))
