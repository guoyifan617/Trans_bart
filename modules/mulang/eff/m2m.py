...
from modules.mulang.m2m import CrossAttn as CrossAttnBase, PositionwiseFF as PositionwiseFFBase, SelfAttn as SelfAttnBase
...
class PositionwiseFF(PositionwiseFFBase):
	...
	def forward(self, x, weight=None, taskid=None, **kwargs):
		...
		if weight is not None:
			...
			out = out.transpose(-1, -2).contiguous().view(-1, self.ngroup).mm(weight).view(_osize).transpose(-1, -2).contiguous()
...
class SelfAttn(SelfAttnBase):
	...
	def forward(self, iQ, mask=None, states=None, **kwargs):#, weight=None
		...
		#if weight is not None:
		#	out = out.transpose(-1, -2).contiguous().view(-1, ngroup).mm(weight).view(bsize, nquery, self.osize, ngroup).transpose(-1, -2).contiguous()
