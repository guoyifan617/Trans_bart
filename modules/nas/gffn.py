...
class Cell(nn.Module):
	...
	def __init__(self, num_node, isize, dropout=0.0, custom_act=use_adv_act_default, enable_bias=enable_prev_ln_bias_default, **kwargs):

		super(Cell, self).__init__()

		self.net0 = nn.Sequential(Linear(isize, isize * (num_node - 1)), Custom_Act() if custom_act else nn.ReLU(inplace=True), Dropout(dropout, inplace=inplace_after_Custom_Act)) if dropout > 0.0 else nn.Sequential(Linear(isize, isize * (num_node - 1)), Custom_Act() if custom_act else nn.ReLU(inplace=True))
		self.net1 = nn.Sequential(Linear(isize, isize), Custom_Act() if custom_act else nn.ReLU(inplace=True), Dropout(dropout, inplace=inplace_after_Custom_Act)) if dropout > 0.0 else nn.Sequential(Linear(isize, isize), Custom_Act() if custom_act else nn.ReLU(inplace=True))
