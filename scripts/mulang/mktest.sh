...
python tools/clean/vocab/unk.py $tgtd/$bpef $tgtd/$bpef.clean
rm $tgtd/$bpef

if $debpe; then
	if $spm_bpe; then
		python tools/spm/decode.py --model $tgtd/bpe.model --input_format piece --input $tgtd/$bpef.clean > $rsf

	else
		sed -r 's/(@@ )|(@@ ?$)//g' < $tgtd/$bpef.clean > $rsf
	fi
	rm $tgtd/$bpef.clean
else
	mv $tgtd/$bpef.clean $rsf
fi
rm $tgtd/test.h5
