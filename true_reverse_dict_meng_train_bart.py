...
from cnfg.vocab.plm.bert_ch import pad_id
import cnfg.plm.bart.base_meng_reverse as cnfg
from cnfg.plm.bart.ihyp import *
from cnfg.vocab.plm.bert_ch import vocab_size as cn_vocab_size

from utils.fmt.vocab.token import ldvocab



def my_load_dict(file):
    vcb_zh = {}
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		with torch_autocast(enabled=_use_amp):
			output = model(seq_batch, oi, word_prediction=True)
	...
	with torch_inference_mode():
		for i in tqdm(range(nd), mininterval=tqdm_mininterval):
			...
			with torch_autocast(enabled=use_amp):
				output = model(seq_batch, seq_o.narrow(1, 0, lo), word_prediction=True)
				loss = lossf(output, ot)
				if multi_gpu:
					loss = loss.sum()
...
wkdir = "".join((cnfg.exp_dir, cnfg.data_id, "/", cnfg.group_id, "/", rid, "/step1/"))
mkdir(wkdir)
...
fvocab_i = '/home/yfguo/zh_data_for_vcb/meng2zh/meng_true_reverse_vcb.txt'
vcbi = my_load_dict(fvocab_i)
nwordi = 25729 #25728+1
print('nwordi is :',nwordi)
print(vcbi)
nwordt = cn_vocab_size
...
if pre_trained_m is not None:
	...
	mymodel.load_plm(fix_parameter_name(torch.load(pre_trained_m, map_location="cpu")))#fix_parameter_name返回结果正确
