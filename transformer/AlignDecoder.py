...
class DecoderLayer(DecoderLayerBase):
	...
	def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, ahsize=None, **kwargs):
		...
		self.cross_attn = ResCrossAttn(isize, _ahsize, num_head=num_head, dropout=attn_drop)
...
class Decoder(DecoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, emb_w=None, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindemb=True, forbidden_index=None, share_layer=False, **kwargs):
		...
		super(Decoder, self).__init__(isize, nwd, num_layer, fhsize=_fhsize, dropout=dropout, attn_drop=attn_drop, emb_w=emb_w, num_head=num_head, xseql=xseql, ahsize=_ahsize, norm_output=norm_output, bindemb=bindemb, forbidden_index=forbidden_index, share_layer=False, **kwargs)
	...
	def forward(self, inpute, inputo, src_pad_mask=None, **kwargs):
		...
		_asize = attns.size()
		out = out + attns.view(-1, _asize[-1]).mv(self.tattn_w.softmax(dim=0) if self.tattn_drop is None else self.tattn_drop(self.tattn_w).softmax(dim=0)).view(_asize[:-1]).bmm(inpute)
	...
	def greedy_decode(self, inpute, src_pad_mask=None, max_len=512, fill_pad=False, sample=False):
		...
		for _tmp, net in enumerate(self.nets):
			out, _state, _attn = net(inpute, (None, None,), src_pad_mask, None, out)
			states[_tmp] = _state
			attns.append(_attn)

		attns = torch.cat(attns, dim=1).permute(0, 2, 3, 1).contiguous()
		_asize = attns.size()
		_attn_w = self.tattn_w.softmax(dim=0) if self.tattn_drop is None else self.tattn_drop(self.tattn_w).softmax(dim=0)
		out = out + attns.view(-1, _asize[-1]).mv(_attn_w).view(_asize[:-1]).bmm(inpute)
		...
		for i in range(1, max_len):
			...
			attns = []
			for _tmp, net in enumerate(self.nets):
				out, _state, _attn = net(inpute, states[_tmp], src_pad_mask, None, out)
				states[_tmp] = _state
				attns.append(_attn)
	...
	def beam_decode(self, inpute, src_pad_mask=None, beam_size=8, max_len=512, length_penalty=0.0, return_all=False, clip_beam=clip_beam_with_lp, fill_pad=False):
		...
		for step in range(1, max_len):
			...
			attns = []
			for _tmp, net in enumerate(self.nets):
				out, _state, _attn = net(inpute, states[_tmp], _src_pad_mask, None, out)
				states[_tmp] = _state
				attns.append(_attn)
