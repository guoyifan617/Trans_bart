...
#from modules.caffn import UniCFFN as PositionwiseFF
...
class DecoderLayer(DecoderLayerBase):
	...
	def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, ahsize=None, norm_residual=norm_residual_default, **kwargs):
		...
		self.ff = HPLSTM(isize, num_head=num_head, osize=isize, fhsize=_fhsize, dropout=dropout)#PositionwiseFF(isize, hsize=_fhsize, dropout=dropout, norm_residual=norm_residual)
	...
	def forward(self, inpute, inputo, src_pad_mask=None, tgt_pad_mask=None, query_unit=None, **kwargs):
		...
		if query_unit is None:
			...
			context, states_return_attn = self.self_attn(query_unit, states=inputo[0])
		...
		if query_unit is None:
			...
			_context, states_return_ffn = self.ff(context, states=inputo[-1])
		...
		if query_unit is None:
			...
			return context, (states_return_attn, states_return_ffn,)
...
class Decoder(DecoderBase):
	...
	def greedy_decode(self, inpute, src_pad_mask=None, max_len=512, fill_pad=False, sample=False):
		...
		for _tmp, net in enumerate(self.nets):
			out, _state = net(inpute, ((None, None,), "init",), src_pad_mask=src_pad_mask, tgt_pad_mask=None, query_unit=out)
			states[_tmp] = _state
