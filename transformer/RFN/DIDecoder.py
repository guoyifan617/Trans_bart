...
from modules.rfn import DualInputLSTMCell4FFN as LSTMCell4FFN#, DualInputLSTMCell4RNMT
...
class DecoderLayer(DecoderLayerBase):
	...
	def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, act_drop=None, num_head=8, ahsize=None, norm_residual=norm_residual_default, k_rel_pos=use_k_relative_position_decoder, **kwargs):

		_ahsize = parse_none(ahsize, isize)
		_fhsize = _ahsize * 4 if fhsize is None else fhsize

		super(DecoderLayer, self).__init__(isize, fhsize=_fhsize, dropout=dropout, attn_drop=attn_drop, act_drop=act_drop, num_head=num_head, ahsize=_ahsize, norm_residual=norm_residual, k_rel_pos=k_rel_pos, **kwargs)
	...
	def forward(self, inpute, inputo, cellin, src_pad_mask=None, tgt_pad_mask=None, query_unit=None, **kwargs):

		if query_unit is None:
			_inputo, lstmo = self.layer_normer1(inputo), inputo
			...
			_query_unit, lstmo = self.layer_normer1(query_unit), query_unit
		...
		out, cellout = self.ff(osattn, _context_new, (lstmo, cellin))
...
class Decoder(DecoderBase):
	...
	def forward(self, inpute, inputo, src_pad_mask=None, **kwargs):
		...
		for net in self.nets:
			out, cell = net(inpute, out, cell, src_pad_mask, _mask)
	...
	def greedy_decode(self, inpute, src_pad_mask=None, max_len=512, fill_pad=False, sample=False):
		...
		cell = out#cell_step = self.init_cx.expand_as(out)

		states = {}

		for _tmp, net in enumerate(self.nets):
			out, _state, cell = net(inpute, (None, None,), cell, src_pad_mask, None, out)
			states[_tmp] = _state
		...
		for i in range(1, max_len):
			...
			for _tmp, net in enumerate(self.nets):
				out, _state, cell = net(inpute, states[_tmp], cell, src_pad_mask, None, out)
				states[_tmp] = _state
	...
	def beam_decode(self, inpute, src_pad_mask=None, beam_size=8, max_len=512, length_penalty=0.0, return_all=False, clip_beam=clip_beam_with_lp, fill_pad=False):
		...
		#cell_step = self.init_cx.expand(real_bsize, 1, isize)
		...
		for step in range(1, max_len):
			...
			for _tmp, net in enumerate(self.nets):
				out, _state, cell = net(inpute, states[_tmp], cell, _src_pad_mask, None, out)
				states[_tmp] = _state
