...
class Decoder(DecoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, emb_w=None, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindemb=True, forbidden_index=None, nprev_context=2, num_layer_cross=1, drop_tok=None, **kwargs):
		...
		self.gdec = nn.ModuleList([DecoderLayer(isize, _fhsize, dropout, attn_drop, num_head, _ahsize) for i in range(num_layer_cross)])
		...
		_dtok = dropout if drop_tok is None else drop_tok
		self.tokdrop = TokenDropout(_dtok, keep_magnitude=True) if _dtok > 0.0 else None
	...
	def forward(self, inpute, inputo, inputot, enc_context, context, src_pad_mask=None, context_mask=None, inputot_mask=None, enc_context_mask=None, **kwargs):
		...
		if self.pemb is not None:
			out = self.pemb(out.select(-1, 0), expand=False).add(out, alpha=sqrt(isize))
		if self.drop is not None:
			out = self.drop(out)
		...
		inputo_context = torch.cat((dece, self.wemb(inputot) if self.tokdrop is None else self.tokdrop(torch.cat((out_emb.narrow(2, 1, nquery - 1), self.wemb(inputot.narrow(2, -1, 1)),), dim=2)),), dim=-1)
		dec_contexts = []
		dec_context_masks = []
		_isize_ctx = isize * 2
	...
	def decode(self, inpute, inputo, enc_context, context, src_pad_mask=None, context_mask=None, beam_size=1, max_len=512, length_penalty=0.0, fill_pad=False):

		return self.beam_decode(inpute, inputo, enc_context, context, src_pad_mask, context_mask, beam_size, max_len, length_penalty, fill_pad=fill_pad) if beam_size > 1 else self.greedy_decode(inpute, inputo, enc_context, context, src_pad_mask, context_mask, max_len, fill_pad=fill_pad)
	...
	def greedy_decode(self, inpute, inputo, enc_context, context, src_pad_mask=None, context_mask=None, max_len=512, fill_pad=False, sample=False):
		...
		inputo_context = torch.cat((dece, out_emb.narrow(2, 1, lo),), dim=-1)
		inputot_mask = inputo.narrow(2, 1, lo).eq(0).unsqueeze(1)
		dec_contexts = []
		dec_context_masks = []
		_isize_ctx = isize * 2
		_nsent_context_base = nsent - self.nprev_context
		...
		_nsent_o = nsent - 1
		inpute, _src_pad_mask = clear_pad_mask(inpute.view(bsize, nsent, -1, isize).narrow(1, 1, _nsent_o), src_pad_mask.narrow(2, 1, _nsent_o), dim=2, mask_dim=-1)
		bsize *= _nsent_o
