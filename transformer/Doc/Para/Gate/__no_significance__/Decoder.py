...
class Decoder(DecoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, act_drop=None, emb_w=None, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindemb=True, forbidden_index=None, num_layer_cross=1, **kwargs):
		...
		self.gdec = nn.ModuleList([DecoderLayer(isize, _fhsize, dropout, attn_drop, act_drop, num_head, _ahsize, ncross=2) for i in range(num_layer_cross)])
	...
	def forward(self, inpute, inputo, enc_context, context, src_pad_mask=None, context_mask=None, **kwargs):

		out = self.wemb(inputo)
		bsize, nsent, nquery, isize = out.size()
		sbsize = bsize * nsent
		out = out.view(sbsize, nquery, isize)
		if self.pemb is not None:
			out = self.pemb(out, expand=False).add(out, alpha=sqrt(out.size(-1)))
		...
		for net in self.gdec:
			out = net(inpute, out, (context, enc_context,), _src_pad_mask, _mask, (context_mask, _src_pad_mask,))
	...
	def decode(self, inpute, enc_context, context, src_pad_mask=None, context_mask=None, beam_size=1, max_len=512, length_penalty=0.0, fill_pad=False):

		return self.beam_decode(inpute, enc_context, context, src_pad_mask, context_mask, beam_size, max_len, length_penalty, fill_pad=fill_pad) if beam_size > 1 else self.greedy_decode(inpute, enc_context, context, src_pad_mask, context_mask, max_len, fill_pad=fill_pad)

	def greedy_decode(self, inpute, enc_context, context, src_pad_mask=None, context_mask=None, max_len=512, fill_pad=False, sample=False):
		...
		for _tmp, net in enumerate(self.gdec, _tmp + 1):
			out, _state = net(inpute, (None, None,), (context, enc_context,), src_pad_mask, None, (context_mask, src_pad_mask,), out)
			states[_tmp] = _state
		...
		for i in range(1, max_len):
			...
			for _tmp, net in enumerate(self.gdec, _tmp + 1):
				out, _state = net(inpute, states[_tmp], (context, enc_context,), src_pad_mask, None, (context_mask, src_pad_mask,), out)
				states[_tmp] = _state
	...
	def beam_decode(self, inpute, enc_context, context, src_pad_mask=None, context_mask=None, beam_size=8, max_len=512, length_penalty=0.0, return_all=False, clip_beam=clip_beam_with_lp, fill_pad=False):
		...
		_src_pad_mask = src_pad_mask.repeat(1, beam_size, 1).view(real_bsize, 1, seql)
		...
		for step in range(1, max_len):
			...
			for _tmp, net in enumerate(self.gdec, _tmp + 1):
				out, _state = net(inpute, states[_tmp], (_context, enc_context,), _src_pad_mask, None, (_context_mask, _src_pad_mask,), out)
				states[_tmp] = _state
	...
	def get_loaded_para_models(self):
		...
		rs = [self.wemb, self.nets, self.classifier]
		if self.out_normer is not None:
