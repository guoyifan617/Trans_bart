...
from transformer.MDoc.Decoder import Decoder
from transformer.MDoc.Encoder import Encoder
from transformer.NMT import NMT as NMTBase
from utils.doc.mono.base4torch import merge_sents
...
class NMT(NMTBase):
	...
	def forward(self, inpute, inputo, mask=None, rsentind=None, psind=None, **kwargs):
		...
		return self.dec(ence, inputo, _mask, rsentind=rsentind, psind=psind)
	...
	def decode(self, inpute, beam_size=1, max_len=None, length_penalty=0.0, rsentind=None):
		...
		ence, mask = merge_sents(ence, mask.squeeze(1))
		mask = mask.unsqueeze(1)

		return self.dec.decode(ence, mask, beam_size, _max_len, length_penalty, rsentind=rsentind)
