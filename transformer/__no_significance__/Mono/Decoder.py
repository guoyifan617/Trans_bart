...
class Decoder(DecoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, emb_w=None, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindemb=True, forbidden_index=None, lang_emb=None, **kwargs):

		_ahsize = isize if ahsize is None else ahsize
		_fhsize = _ahsize * 4 if fhsize is None else fhsize

		super(Decoder, self).__init__(isize, nwd, num_layer, fhsize=_fhsize, dropout=dropout, attn_drop=attn_drop, emb_w=emb_w, num_head=num_head, xseql=xseql, ahsize=_ahsize, norm_output=norm_output, bindemb=bindemb, None, **kwargs)
	...
	def decode(self, inpute, src_pad_mask, beam_size=1, max_len=512, length_penalty=0.0, lang_id=0, fill_pad=False):

		return self.beam_decode(inpute, src_pad_mask, beam_size, max_len, length_penalty, lang_id, fill_pad=fill_pad) if beam_size > 1 else self.greedy_decode(inpute, src_pad_mask, max_len, lang_id, fill_pad=fill_pad)

	def greedy_decode(self, inpute, src_pad_mask=None, max_len=512, lang_id=0, fill_pad=False, sample=False):
	...
	def beam_decode(self, inpute, src_pad_mask=None, beam_size=8, max_len=512, length_penalty=0.0, lang_id=0, return_all=False, clip_beam=clip_beam_with_lp, fill_pad=False):
