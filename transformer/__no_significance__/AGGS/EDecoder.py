...
class Decoder(DecoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, emb_w=None, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindemb=True, forbidden_index=None, num_nod=6, **kwargs):

		_ahsize = parse_none(ahsize, isize)

		_fhsize = _ahsize * 4 if fhsize is None else fhsize

		super(Decoder, self).__init__(isize, nwd, num_layer, fhsize=_fhsize, dropout=dropout, attn_drop=attn_drop, emb_w=emb_w, num_head=num_head, xseql=xseql, ahsize=ahsize, norm_output=norm_output, bindemb=bindemb, forbidden_index=forbidden_index, **kwargs)
		...
		self.layer_cost = (isize * isize * 16 + isize * 11) / 1e6
		...
		for _cnode, (node, node_act, act_weight,) in enumerate(zip(self.nodes, _nsel.tolist(), _nw.unbind(0))):
			...
			if not _eid in processed_nodes:
				...
				if _eid < 0 and _eid > _node_id:
					while _node_id < _eid:
						out = next(nets)(inpute, out, src_pad_mask, _mask)
						_node_id += 1
						nodes_output[_node_id] = out
