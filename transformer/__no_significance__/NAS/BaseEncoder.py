...
class EncoderLayer(nn.Module):
	...
	def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, ahsize=None, norm_residual=norm_residual_default, num_nod=4, max_prev_nodes=4, edge_p=None, norm_output=True, base_cost_rate=2.0, **kwargs):
...
class Encoder(EncoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, num_nod=4, max_prev_nodes=4, **kwargs):
		...
		self.nets = nn.ModuleList([EncoderLayer(isize, _fhsize, dropout, attn_drop, num_head, _ahsize, False, num_nod, max_prev_nodes, self.edge_p) for i in range(num_layer)])
