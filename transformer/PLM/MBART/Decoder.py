...
from cnfg.plm.mbart.base import remove_classifier_bias
from cnfg.plm.mbart.ihyp import *
from cnfg.vocab.plm.mbart import eos_id, pad_id, pemb_start_ind

class DecoderLayer(DecoderLayerBase):

	def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, act_drop=None, num_head=8, ahsize=None, norm_residual=norm_residual_default, k_rel_pos=use_k_relative_position_decoder, max_bucket_distance=relative_position_max_bucket_distance_decoder, model_name="model.decoder", **kwargs):
...
class Decoder(DecoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, act_drop=None, emb_w=None, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindemb=True, forbidden_index=None, share_layer=False, disable_pemb=disable_std_pemb_decoder, model_name="model.decoder", **kwargs):
	...
	def forward(self, inpute, inputo, src_pad_mask=None, word_prediction=False, **kwargs):
		...
		if self.pemb is not None:
			out = self.pemb.narrow(0, pemb_start_ind, nquery).add(out, alpha=sqrt(out.size(-1)))
	...
	def greedy_decode(self, inpute, src_pad_mask=None, max_len=512, fill_pad=False, sample=False, lang_id=None, **kwargs):

		bsize = inpute.size(0)

		out = self.get_sos_emb(inpute, lang_id=lang_id)
		if self.pemb is not None:
			sqrt_isize = sqrt(out.size(-1))
			out = self.pemb[pemb_start_ind].add(out, alpha=sqrt_isize)
		out = self.emb_normer(out)
		if self.drop is not None:
			out = self.drop(out)
		...
		for i in range(1, max_len):
			...
			out = self.wemb(wds)
			if self.pemb is not None:
				out = self.pemb[pemb_start_ind + i].add(out, alpha=sqrt_isize)
			out = self.emb_normer(out)
			if self.drop is not None:
				out = self.drop(out)
	...
	def beam_decode(self, inpute, src_pad_mask=None, beam_size=8, max_len=512, length_penalty=0.0, return_all=False, clip_beam=clip_beam_with_lp, fill_pad=False, lang_id=None, **kwargs):
		...
		for step in range(1, max_len):
			...
			out = self.wemb(wds)
			if self.pemb is not None:
				out = self.pemb[pemb_start_ind + step].add(out, alpha=sqrt_isize)
			out = self.emb_normer(out)
			if self.drop is not None:
				out = self.drop(out)
	...
	# MBART starts decoding with the language ID
	def get_sos_emb(self, inpute, bsize=None, lang_id=None):

		if isinstance(lang_id, torch.Tensor):
			return self.wemb(lang_id).unsqueeze(1)
		else:
	...
	def load_plm(self, plm_parameters, model_name=None, **kwargs):
		...
		# MBART does NOT have the bias vector in the classifier
