...
class Decoder(DecoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, emb_w=None, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindemb=True, forbidden_index=None, ntask=None, ngroup=None, task_emb_w=None, share_layer=False, **kwargs):
		...
		if share_layer:
			...
			self.nets = nn.ModuleList([DecoderLayer(isize, _fhsize, dropout, attn_drop, num_head, _ahsize, ngroup=ngroup, ntask=ntask, expand_layer=True, merge_layer=False)] + [_shared_layer for i in range(num_layer - 2)] + [DecoderLayer(isize, _fhsize, dropout, attn_drop, num_head, _ahsize, ngroup=ngroup, expand_layer=False, merge_layer=True)])
		else:
