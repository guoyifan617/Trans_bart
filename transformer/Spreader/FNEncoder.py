...
class Encoder(EncoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, share_layer=False, disable_pemb=disable_std_pemb_encoder, s_start=2, s_end=8, e_start=4, e_end=16, **kwargs):
		...
		if share_layer:
			_shared_layer = EncoderLayer(isize, _fhsize, dropout, attn_drop, num_head, _ahsize, start=s_start, end=s_end)
			self.nets = nn.ModuleList([_shared_layer for i in range(num_layer)])
		else:
			self.nets = nn.ModuleList([EncoderLayer(isize, _fhsize, dropout, attn_drop, num_head, _ahsize, start=_s_start, end=_s_end) for (_s_start, _s_end,) in zip(grow_func(s_start, e_start, num_layer), grow_func(s_end, e_end, num_layer))])
