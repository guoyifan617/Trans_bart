...
from transformer.NMT import NMT as StdNMT
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	tgt_grp = td["src"]
	global src_tgt_m, tgt_src_m, beam_size, length_penalty, sos_id
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		seq_o = torch.from_numpy(tgt_grp[i_d][()])
		lo = seq_o.size(1) - 1
		sos_pad = torch.full((seq_o.size(0), 1,), sos_id, dtype=torch.long, device=mv_device if mv_device else None)
		if mv_device:
	...
	with torch_no_grad():
		for i in tqdm(range(nd), mininterval=tqdm_mininterval):
			...
			if mv_device:
				...
				seq_mt = seq_mt.to(mv_device, non_blocking=True)
				seq_o = seq_o.to(mv_device, non_blocking=True)
...
src_tgt_m, tgt_src_m = StdNMT(cnfg.isize, nwordi, nwordt, cnfg.nlayer_mt, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.share_emb, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb, cnfg.forbidden_indexes), StdNMT(cnfg.isize, nwordt, nwordi, cnfg.nlayer_mt, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.share_emb, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb, cnfg.tsm_forbidden_indexes)
src_tgt_m, tgt_src_m = init_model_params(src_tgt_m), init_model_params(tgt_src_m)
src_tgt_m.apply(init_fixing)
tgt_src_m.apply(init_fixing)
logger.info("Load round-trip models from: %s, %s" %(cnfg.src_tgt_m, cnfg.tgt_src_m,))
src_tgt_m, tgt_src_m = load_model_cpu(cnfg.src_tgt_m, src_tgt_m), load_model_cpu(cnfg.tgt_src_m, tgt_src_m)

beam_size, length_penalty = cnfg.beam_size, cnfg.length_penalty
...
freeze_module(src_tgt_m)
freeze_module(tgt_src_m)
...
if cuda_device:
	...
	src_tgt_m.to(cuda_device, non_blocking=True)
	tgt_src_m.to(cuda_device, non_blocking=True)
