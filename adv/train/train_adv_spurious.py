...
from modules.base import GradientReversalLayer
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		spu_seq_batch = build_rand_batch(seq_batch, nwordi)[0]
		spu_seq_o, spu_pad_mask = build_rand_batch(seq_o, nwordt)
		spu_oi = spu_seq_o.narrow(1, 0, lo)
		with torch_autocast(enabled=_use_amp):
			output = model(spu_seq_batch, spu_oi)
			spu_ot = output.detach().argmax(-1).masked_fill_(spu_pad_mask, pad_id)
			loss = grl(lossf(output, spu_ot))

		if scaler is None:
			loss.backward()
		else:
			scaler.scale(loss).backward()
		loss = output = spu_oi = spu_ot = spu_seq_batch = spu_seq_o = spu_pad_mask = None
