...
from transformer.Probe.SEncoder import Encoder as NMT
			...
			if _cur_rstep is not None:
				if save_checkp_epoch and (save_every is not None) and (_cur_rstep % save_every == 0) and (chkpf is not None) and (_cur_rstep > 0):
					if num_checkpoint > 1:
						...
						_chkpf = chkpf
					save_model(model.module.classifier if multi_gpu else model.classifier, _chkpf, False, print_func=logger.info)
...
if fine_tune_m is not None:
	...
	freeze_module(_tmpm)
	mymodel.load_base(_tmpm.enc)
	_tmpm = None

lossf = LabelSmoothingLoss(nwordi, cnfg.label_smoothing, ignore_index=[pad_id, sos_id, eos_id], reduction="sum", forbidden_index=cnfg.forbidden_indexes)
...
if fine_tune_m is None:
	save_model(mymodel.module.classifier if multi_gpu else mymodel.classifier, wkdir + "init.h5", False, print_func=logger.info)
	...
	if cnt_states is not None:
		...
		save_model(mymodel.module.classifier if multi_gpu else mymodel.classifier, wkdir + "train_0_%.3f_%.3f_%.2f.h5" % (tminerr, vloss, vprec), False, print_func=logger.info, mtyp=("eva" if overwrite_eva else "train") if save_auto_clean else None)
		if statesf is not None:
...
for i in range(1, maxrun + 1):
	...
	if (vprec <= minerr) or (vloss <= minloss):
		save_model(mymodel.module.classifier if multi_gpu else mymodel.classifier, wkdir + "eva_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec), False, print_func=logger.info, mtyp="eva" if save_auto_clean else None)
		if statesf is not None:
			save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
		logger.info("New best model saved")
		...
		if terr < tminerr:
			tminerr = terr
			save_model(mymodel.module.classifier if multi_gpu else mymodel.classifier, wkdir + "train_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec), False, print_func=logger.info, mtyp=("eva" if overwrite_eva else "train") if save_auto_clean else None)
			if statesf is not None:
				save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
		elif epoch_save:
			save_model(mymodel.module.classifier if multi_gpu else mymodel.classifier, wkdir + "epoch_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec), False, print_func=logger.info)
...
save_model(mymodel.module.classifier if multi_gpu else mymodel.classifier, wkdir + "last.h5", False, print_func=logger.info)
if statesf is not None:
	save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
