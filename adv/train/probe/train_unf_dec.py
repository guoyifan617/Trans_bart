...
from transformer.Probe.UnfNMT import NMT
			...
			if _cur_rstep is not None:
				if save_checkp_epoch and (save_every is not None) and (_cur_rstep % save_every == 0) and (chkpf is not None) and (_cur_rstep > 0):
					if num_checkpoint > 1:
						...
						_chkpf = chkpf
					save_model(model.module.dec.nets if multi_gpu else model.dec.nets, _chkpf, False, print_func=logger.info)
...
if cnfg.tgt_emb is not None:
	...
	load_emb(cnfg.tgt_emb, mymodel.dec.nets.wemb.weight, nwordt, cnfg.scale_down_emb, cnfg.freeze_tgtemb)
...
if fine_tune_m is None:
	save_model(mymodel.module.dec.nets if multi_gpu else mymodel.dec.nets, wkdir + "init.h5", False, print_func=logger.info)
	...
	if cnt_states is not None:
		...
		save_model(mymodel.module.dec.nets if multi_gpu else mymodel.dec.nets, wkdir + "train_0_%.3f_%.3f_%.2f.h5" % (tminerr, vloss, vprec), False, print_func=logger.info, mtyp=("eva" if overwrite_eva else "train") if save_auto_clean else None)
		if statesf is not None:
...
for i in range(1, maxrun + 1):
	...
	if (vprec <= minerr) or (vloss <= minloss):
		save_model(mymodel.module.dec.nets if multi_gpu else mymodel.dec.nets, wkdir + "eva_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec), False, print_func=logger.info, mtyp="eva" if save_auto_clean else None)
		if statesf is not None:
			save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
		logger.info("New best model saved")
		...
		if terr < tminerr:
			tminerr = terr
			save_model(mymodel.module.dec.nets if multi_gpu else mymodel.dec.nets, wkdir + "train_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec), False, print_func=logger.info, mtyp=("eva" if overwrite_eva else "train") if save_auto_clean else None)
			if statesf is not None:
				save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
		elif epoch_save:
			save_model(mymodel.module.dec.nets if multi_gpu else mymodel.dec.nets, wkdir + "epoch_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec), False, print_func=logger.info)
...
save_model(mymodel.module.dec.nets if multi_gpu else mymodel.dec.nets, wkdir + "last.h5", False, print_func=logger.info)
if statesf is not None:
	save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
