...
from loss.mulang import MultiLabelSmoothingLoss as LabelSmoothingLoss
...
def back_translate(model, seq_in, taskid, beam_size, multi_gpu, enable_torch_autocast=False, step_bsize=32, step_ntok=640, pivot_bt=True):
	...
	with torch_inference_mode(), torch_autocast(enabled=enable_torch_autocast):
		while sind < bsize:
			...
			if pivot_bt:
				_tid = taskid.narrow(0, sind, num_narrow)
				out = model.decode(seq_in.narrow(0, sind, num_narrow), taskid=_tid.new_zeros(_tid.size(), dtype=_tid.dtype, device=_tid.device), beam_size=beam_size, max_len=_max_len).detach()
				rs.append(model.decode(torch.cat((torch.ones(num_narrow, 1, dtype=out.dtype, device=out.device), out,), dim=1), taskid=_tid, beam_size=beam_size, max_len=_max_len).detach())
				out = _tid = None
			else:
				rs.append(model.decode(seq_in.narrow(0, sind, num_narrow), taskid=taskid.narrow(0, sind, num_narrow), beam_size=beam_size, max_len=_max_len).detach())
			sind += num_narrow
	if multi_gpu:
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		seq_t = torch.from_numpy(task_grp[i_d][()])
		lo = seq_o.size(1) - 1
		if mv_device:
			seq_o = seq_o.to(mv_device, non_blocking=True)
			seq_t = seq_t.to(mv_device, non_blocking=True)
		seq_o, seq_t = seq_o.long(), seq_t.long()

		_bt_taskid = torch.randint(t_sample_max_id, seq_t.size(), dtype=seq_t.dtype, device=seq_t.device)
		_bt_taskid[_bt_taskid.ge(seq_t)] += 1
		...
		with torch_autocast(enabled=_use_amp):
			output = model(seq_batch, oi, taskid=seq_t)
			loss = lossf(output, ot, seq_t)
			if multi_gpu:
				loss = loss.sum()
	...
	with torch_inference_mode():
		for i in tqdm(range(nd), mininterval=tqdm_mininterval):
			...
			with torch_autocast(enabled=use_amp):
				output = model(seq_batch, seq_o.narrow(1, 0, lo), taskid=seq_t)
				loss = lossf(output, ot, seq_t)
				if multi_gpu:
					loss = loss.sum()
