...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	def merge_ind(indl, mvdev):

		rsl = []
		for indt in zip(*indl):
			tmp = []
			_acc_ind = 0
			for indu in indt:
				if _acc_ind > 0:
					_tmp = indu.to(mvdev, non_blocking=True)
					tmp.append(_tmp + _acc_ind)
					_acc_ind += _tmp.size(0)
				else:
					tmp.append(indu)
					_acc_ind += indu.size(0)
			rsl.append(torch.cat(tmp, dim=0))
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		with torch_autocast(enabled=_use_amp):
			output, rind = model(seq_batch, oi)
			_tind = merge_ind(rind, mv_device) if multi_gpu else rind
			ot = torch.stack([ot if _ind is None else ot.index_select(0, _ind) for _ind in [None] + _tind], 1)
...
mymodel = NMT(cnfg.isize, nwordi, nwordt, cnfg.nlayer, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.share_emb, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb, cnfg.forbidden_indexes, nmimo=cnfg.nmimo)
