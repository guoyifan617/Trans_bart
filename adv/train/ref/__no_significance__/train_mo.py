...
from lrsch import GoogleLR as LRScheduler, InverseSqrtLR
...
from utils.muloptm import getlr as getmolr, optm_step, report_dl
...
loss_batch, token_batch, naccdl, batch_data, accdl = 0.0, 0, 0, [], []

def eva_batch(model, lossf, datain):
	_t_state = model.training
	model.eval()
	sum_loss = 0.0
	w = 0
	with torch_no_grad():
		for seq_batch, seq_o in datain:
			...
			w += ot.ne(0).int().sum().item()
			loss = output = ot = None
	model.train(_t_state)
	return sum_loss / float(w)
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, use_amp=False):
	...
	global loss_batch, token_batch, naccdl, batch_data, accdl
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		batch_data.append((seq_batch, seq_o,))
		...
		if _done_tokens >= tokens_optm:
			if multi_gpu:
				...
				_mom = model
				_mol = lossf
			_aodl = optm_step(_mom, optm, _mol, eva_batch, batch_data, loss_batch / float(token_batch))
			optm[0].zero_grad(set_to_none=optm_step_zero_grad_set_none)
			if multi_gpu:
				model.update_replicas()
			accdl = [au + ao for au, ao in zip(accdl, _aodl)] if accdl else _aodl
			naccdl += 1
			loss_batch, token_batch, batch_data = 0.0, 0, []
			_done_tokens = 0
		...
		if nreport is not None:
			...
			if cur_b % nreport == 0:
				...
				logger.info(report_dl(accdl, naccdl))
				accdl, naccdl = [], 0
				part_loss = 0.0
				part_wd = 0
...
#, momentum=0.9
optimizer = [Optimizer(mymodel.parameters(), lr=init_lr, betas=adam_betas_default, eps=ieps_adam_default, weight_decay=cnfg.weight_decay, amsgrad=use_ams), optim.RMSprop(mymodel.parameters(), lr=init_lr), optim.SGD(mymodel.parameters(), lr=init_lr)]
optimizer[0].zero_grad(set_to_none=optm_step_zero_grad_set_none)
...
logger.info("".join(("Init lr: ", ",".join(iter_to_str(getmolr(optimizer))), ", Dev Loss/Error: %.3f %.2f" % (minloss, minerr))))
		...
		if namin >= earlystop:
			if done_tokens > 0:
				if multi_gpu:
					...
					_mom = mymodel
					_mol = lossf
				optm_step(_mom, optm, _mol, eva_batch, batch_data, loss_batch / float(token_batch))
				done_tokens = 0
