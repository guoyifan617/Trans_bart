...
from transformer.BERT.Std import NMT
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, use_amp=False):
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		loss = lossf(output, seq_batch, sample_mask=_sample_mask)
		if multi_gpu:
			loss = loss.sum()
		...
		_dmask = seq_batch.ne(0)
		wd_add = _dmask.int().sum().item()
		wd_add_loss = (_dmask & _sample_mask).int().sum().item()
		loss = output = seq_batch = _sample_mask = _dmask = _sample_base = None
		sum_loss += loss_add
		if save_loss:
			_ls[i_d] = loss_add / wd_add_loss
		sum_wd += wd_add_loss
		_done_tokens += wd_add
		...
		if nreport is not None:
			...
			if cur_b % nreport == 0:
				if report_eva:
					_leva, _eeva = eva(td, sample(tl, nbeva), model, lossf, mv_device, multi_gpu)
...
def eva(ed, vl, model, lossf, mv_device, multi_gpu):
	r = 0
	w = 0
	sum_loss = 0.0
	model.eval()
	...
	with torch_inference_mode():
		if multi_gpu:
			...
			_evam, _evaloss = model, lossf
		for bid in tqdm(vl, mininterval=tqdm_mininterval):
...
mymodel = NMT(cnfg.isize, nwordi, cnfg.nlayer, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb, cnfg.forbidden_indexes)
...
minloss, minerr = eva(td, sample(tl, nbeva), mymodel, lossf, cuda_device, multi_gpu)
...
if fine_tune_m is None:
	...
	if cnt_states is not None:
		...
		if "training_list" in _remain_states:
			...
			shuffle(tl)
			_ctl = tl
		tminerr, done_tokens, cur_checkid, remain_steps, _ = train(td, _ctl, td, None, optimizer, lrsch, mymodel, lossf, cuda_device, logger, done_tokens, multi_gpu, tokens_optm, batch_report, save_every, chkpf, state_holder, statesf, num_checkpoint, cur_checkid, report_eva, remain_steps, False, False, scaler)
		vloss, vprec = eva(td, sample(tl, nbeva), mymodel, lossf, cuda_device, multi_gpu)
...
for i in range(1, maxrun + 1):
	shuffle(tl)
	free_cache(use_cuda)
	terr, done_tokens, cur_checkid, remain_steps, _Dws = train(td, tl, td, None, optimizer, lrsch, mymodel, lossf, cuda_device, logger, done_tokens, multi_gpu, tokens_optm, batch_report, save_every, chkpf, state_holder, statesf, num_checkpoint, cur_checkid, report_eva, remain_steps, dss_ws > 0, i >= start_chkp_save, scaler)
	vloss, vprec = eva(td, sample(tl, nbeva), mymodel, lossf, cuda_device, multi_gpu)
