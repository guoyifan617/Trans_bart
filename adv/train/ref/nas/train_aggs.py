...
from transformer.AGGS.NMT import NMT
...
class geTau:
	...
	def __init__(self, k=512, **kwargs):
	...
	def __call__(self):
		...
		return max(min(1.0 / sqrt(self.cur_step / self.k), 1.0), 0.01)
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, use_amp=False, lrsch_arch=None, train_arch=False, nas_steps_remain=0, arch_design=None, arch_stable=0, arch_steps=100, model_steps=500, arch_early_stop=128, cost_weight=0.5, tausch=None, start_nas_step=8000):
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		if _train_arch:
			output, arch_loss = model(seq_batch, oi)#, sel
		else:
			output = model(seq_batch, oi)#, sel
		loss = lossf(output, ot)
		if multi_gpu:
			loss = loss.sum()
			if _train_arch and arch_loss:
				_al = []
				for au in arch_loss:
					if au is not None:
						_al.append(au)
				arch_loss = gather(_al, mv_device, 0).sum() if _al else None
...
if fine_tune_m is not None:
	...
	mymodel.enc.load_base(_tmpm.enc)#mymodel.enc = _tmpm.enc
	mymodel.dec.load_base(_tmpm.dec)#mymodel.dec = _tmpm.dec
	_tmpm = None
...
tausch = geTau(max(1, _warm_arch // 2) if cnfg.tau_step is None else cnfg.tau_step)
