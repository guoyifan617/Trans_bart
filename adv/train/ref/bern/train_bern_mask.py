...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, use_amp=False):
	...
	global enable_bern_steps, cnfg, wkdir
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		if _done_tokens >= tokens_optm:
			...
			if _cur_rstep is not None:
				...
				if _cur_rstep <= 0:
					break
				if _cur_rstep < enable_bern_steps:
					logger.info("Switch to Bernoulli SuperMask training.")
					save_model(model, wkdir + "warm.h5", multi_gpu, print_func=logger.info)
					mymodel.useBernMask(True)
					enable_bern_steps = -1
					mymodel.train_parameters(False)
					lrsch.cur_step = 0
					_cur_rstep = cnfg.training_steps
					reinit_Adam(optm)
			lrsch.step()
...
enable_bern_steps = None if remain_steps is None else ceil(remain_steps * (1.0 - warm_ratio))
...
if enable_bern_steps is not None:
	mymodel.useBernMask(False)
	if (warm_ratio is not None) and (warm_ratio <= 0.0):
		logger.info("Switch to Bernoulli SuperMask training.")
		mymodel.useBernMask(True)
		enable_bern_steps = -1
		mymodel.train_parameters(False)
