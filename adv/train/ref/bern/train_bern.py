...
from parallel.parallelBern import DataParallelMT
...
from utils.prune import bern_thres, force_mask_data, force_mask_grad, init_model_bernparams, prune_bern, sort_mask_para
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		if _done_tokens >= tokens_optm:
			if multi_gpu:
				...
				force_mask_grad(model.module)
				optm.step()
				optm.zero_grad(set_to_none=optm_step_zero_grad_set_none)
				force_mask_data(model.module)
				model.update_replicas()
			else:
				force_mask_grad(model)
				optm.step()
				optm.zero_grad(set_to_none=optm_step_zero_grad_set_none)
				force_mask_data(model)
			_done_tokens = 0
...
if remain_steps is not None:
	remain_steps = ceil(remain_steps * (1.0 - warm_ratio))
...
mymodel = init_model_bernparams(mymodel)
...
if fine_tune_m is not None:
	...
	if prune_ratio is None:
		...
		prune_thres = bern_thres(_tmp, 1.0 - prune_ratio)
		logger.info("Using %f as threshold to prune %.2f%% parameters" % (prune_thres, prune_ratio * 100.0,))
	_tmp = None
	prune_bern(mymodel, prune_thres)
	# with manual call to force_mask_grad and force_mask_data, enable BernoulliParameter.ignore_mask to directly return BernoulliParameter.data
		...
		if namin >= earlystop:
			if done_tokens > 0:
				if multi_gpu:
					...
					force_mask_grad(mymodel.module)
					optimizer.step()
					force_mask_data(mymodel.module)
				else:
					force_mask_grad(mymodel)
					optimizer.step()
					force_mask_data(mymodel)
				done_tokens = 0
