...
mymodel = NMT(cnfg.isize, nwordi, nwordt, cnfg.nlayer, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.share_emb, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb, cnfg.forbidden_indexes, adv_weight=cnfg.adv_weight)
...
logger.info("".join(("Init lr: ", ",".join(tostr(getlr(optimizer[0]))), ", Dev Loss/Error: %.3f %.2f" % (minloss, minerr))))
