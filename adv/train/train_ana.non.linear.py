...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, use_amp=False):
	...
	global minerr, minloss, wkdir, save_auto_clean, namin, model_func, layer_linear
	model.train()
			...
			if _cur_rstep is not None:
				if save_checkp_epoch and (save_every is not None) and (_cur_rstep % save_every == 0) and (chkpf is not None) and (_cur_rstep > 0):
					if num_checkpoint > 1:
						...
						_chkpf = chkpf
					save_model(model_func(model.module if multi_gpu else model).nets[layer_linear], _chkpf, False, print_func=logger.info)
...
_tmpm = MALinear(cnfg.isize, cnfg.isize, bias=False, args_take=cnfg.arg_index, add_out_func=cnfg.add_out_func)
...
optimizer = Optimizer(model_func(mymodel).nets[layer_linear].parameters(), lr=init_lr, betas=adam_betas_default, eps=ieps_adam_default, weight_decay=cnfg.weight_decay, amsgrad=use_ams)
...
if fine_tune_m is None:
	save_model(model_func(mymodel.module if multi_gpu else mymodel).nets[layer_linear], wkdir + "init.h5", False, print_func=logger.info)
	...
	if cnt_states is not None:
		...
		save_model(model_func(mymodel.module if multi_gpu else mymodel).nets[layer_linear], wkdir + "train_0_%.3f_%.3f_%.2f.h5" % (tminerr, vloss, vprec), False, print_func=logger.info, mtyp=("eva" if overwrite_eva else "train") if save_auto_clean else None)
		if statesf is not None:
...
for i in range(1, maxrun + 1):
	...
	if (vprec <= minerr) or (vloss <= minloss):
		save_model(model_func(mymodel.module if multi_gpu else mymodel).nets[layer_linear], wkdir + "eva_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec), False, print_func=logger.info, mtyp="eva" if save_auto_clean else None)
		if statesf is not None:
			save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
		logger.info("New best model saved")
		...
		if terr < tminerr:
			tminerr = terr
			save_model(model_func(mymodel.module if multi_gpu else mymodel).nets[layer_linear], wkdir + "train_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec), False, print_func=logger.info, mtyp=("eva" if overwrite_eva else "train") if save_auto_clean else None)
			if statesf is not None:
				save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
		elif epoch_save:
			save_model(model_func(mymodel.module if multi_gpu else mymodel).nets[layer_linear], wkdir + "epoch_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec), False, print_func=logger.info)
...
save_model(model_func(mymodel.module if multi_gpu else mymodel).nets[layer_linear], wkdir + "last.h5", False, print_func=logger.info)
if statesf is not None:
	save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
