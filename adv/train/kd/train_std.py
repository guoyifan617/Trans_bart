...
from transformer.KDNMT import NMT as NMT
from transformer.NMT import NMT as SNMT
...
import cnfg.kd as cnfg
import cnfg.tbase as cnfgt
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		target_mask = ot.eq(pad_id)
		with torch_autocast(enabled=_use_amp):
			output, kd_loss = model(seq_batch, oi, target_mask=target_mask)
		...
		wd_add = ot.numel() - target_mask.int().sum().item()
		loss = output = oi = ot = seq_batch = seq_o = target_mask = None
		sum_loss += loss_add
		if save_loss:
			...
			if _cur_rstep is not None:
				if save_checkp_epoch and (save_every is not None) and (_cur_rstep % save_every == 0) and (chkpf is not None) and (_cur_rstep > 0):
					if num_checkpoint > 1:
						...
						_chkpf = chkpf
					save_model(model.module.student if multi_gpu else model.student, _chkpf, False, print_func=logger.info)
...
student = SNMT(cnfg.isize, nwordi, nwordt, cnfg.nlayer, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.share_emb, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb, cnfg.forbidden_indexes)

fine_tune_m = cnfg.fine_tune_m

student = init_model_params(student)
student.apply(init_fixing)
if fine_tune_m is not None:
	logger.info("Load pre-trained model from: " + fine_tune_m)
	student = load_model_cpu(fine_tune_m, student)
	student.apply(load_fixing)

teacher = TNMT(cnfgt.isize, nwordi, nwordt, cnfgt.nlayer, cnfgt.ff_hsize, cnfgt.drop, cnfgt.attn_drop, cnfgt.share_emb, cnfgt.nhead, cache_len_default, cnfgt.attn_hsize, cnfgt.norm_output, cnfgt.bindDecoderEmb, cnfgt.forbidden_indexes)

fine_tune_m_teacher = cnfg.fine_tune_m_teacher

teacher = init_model_params(teacher)
teacher.apply(init_fixing)
if fine_tune_m_teacher is not None:
	logger.info("Load pre-trained teacher model from: " + fine_tune_m_teacher)
	teacher = load_model_cpu(fine_tune_m_teacher, teacher)
	teacher.apply(load_fixing)
...
if fine_tune_m is None:
	save_model(mymodel.module.student if multi_gpu else mymodel.student, wkdir + "init.h5", False, print_func=logger.info)
	...
	if cnt_states is not None:
		...
		save_model(mymodel.module.student if multi_gpu else mymodel.student, wkdir + "train_0_%.3f_%.3f_%.2f.h5" % (tminerr, vloss, vprec,), False, print_func=logger.info, mtyp=("eva" if overwrite_eva else "train") if save_auto_clean else None)
		if statesf is not None:
...
for i in range(1, maxrun + 1):
	...
	if (vprec <= minerr) or (vloss <= minloss):
		save_model(mymodel.module.student if multi_gpu else mymodel.student, wkdir + "eva_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec,), False, print_func=logger.info, mtyp="eva" if save_auto_clean else None)
		if statesf is not None:
			save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
		logger.info("New best model saved")
		...
		if terr < tminerr:
			tminerr = terr
			save_model(mymodel.module.student if multi_gpu else mymodel.student, wkdir + "train_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec,), False, print_func=logger.info, mtyp=("eva" if overwrite_eva else "train") if save_auto_clean else None)
			if statesf is not None:
				save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
		elif epoch_save:
			save_model(mymodel.module.student if multi_gpu else mymodel.student, wkdir + "epoch_%d_%.3f_%.3f_%.2f.h5" % (i, terr, vloss, vprec,), False, print_func=logger.info)
			if statesf is not None:
				save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
...
save_model(mymodel.module.student if multi_gpu else mymodel.student, wkdir + "last.h5", False, print_func=logger.info)
if statesf is not None:
	save_states(state_holder.state_dict(update=False, **{"remain_steps": remain_steps, "checkpoint_id": cur_checkid}), statesf, print_func=logger.info)
