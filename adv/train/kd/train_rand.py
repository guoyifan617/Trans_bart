...
#from modules.kd.base import Argmax#, SampleMax
...
from utils.kd.rand import build_rand_batch
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	global kdmodel, kdloss, nwordi, nwordt
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		with torch_no_grad():
			...
			oi = seq_o.narrow(1, 0, seq_o.size(1) - 1)
			with torch_autocast(enabled=_use_amp):
				ot = kdmodel(seq_batch, oi).masked_fill_(_pad_mask.unsqueeze(-1), 0.0)#pad_id

		with torch_autocast(enabled=_use_amp):
			output = model(seq_batch, oi)
			loss = kdloss(output, ot)
			if multi_gpu:
				loss = loss.sum()
...
kdmodel = NMT(cnfg.isize, nwordi, nwordt, cnfg.nlayer, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.share_emb, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb, cnfg.forbidden_indexes)
...
if fine_tune_m is not None:
	...
	kdmodel = load_model_cpu(fine_tune_m, kdmodel)
	kdmodel.apply(load_fixing)

kdmodel.dec.lsm = nn.Softmax(-1)#Argmax(-1)
...
kdloss = nn.KLDivLoss(reduction="sum")
...
if cuda_device:
	...
	kdloss.to(cuda_device, non_blocking=True)
