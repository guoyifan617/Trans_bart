...
from transformer.SelfKD.NMT import NMT
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		wd_add = gold_pad_mask.numel() - gold_pad_mask.int().sum().item()
		...
		if _done_tokens >= tokens_optm:
			...
			if _cur_rstep is not None:
				...
				_cur_rstep -= 1
				if (not use_kd) and (_cur_rstep <= kd_step):
					use_kd = True
				if _cur_rstep <= 0:
					break
			lrsch.step()
		...
		if nreport is not None:
			part_loss += loss_add
			part_loss_kd += loss_add_kd
			part_wd += wd_add
			if cur_b % nreport == 0:
				if report_eva:
					_leva, _eeva = eva(ed, nd, model, lossf, mv_device, multi_gpu, _use_amp)
					logger.info("Average loss over %d tokens: %.3f %.3f, valid loss/error: %.3f %.2f" % (part_wd, part_loss / part_wd, part_loss_kd / part_wd, _leva, _eeva,))
					...
					logger.info("Average loss over %d tokens: %.3f %.3f" % (part_wd, part_loss / part_wd, part_loss_kd / part_wd,))
				part_loss = part_loss_kd = 0.0
				part_wd = 0
