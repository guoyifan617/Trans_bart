...
from lrsch import InverseSqrtLR#, GoogleLR
from parallel.base import DataParallelCriterion
from parallel.optm import MultiGPUGradScaler
from parallel.parallelMT import DataParallelMT
from transformer.MDoc.NMT import NMT
from utils.base import clear_pad, free_cache, get_logger, mkdir, set_random_seed
...
def get_batch(seqin):
	...
	_nsent = seqin.size(1)
	_nsent_in = _nsent - 1
	_sel_sent = randint(0, _nsent_in)

	if _sel_sent == 0:
		_seq_batch = seqin.narrow(1, 1, _nsent_in)
	elif _sel_sent == _nsent_in:
		_seq_batch = seqin.narrow(1, 0, _nsent_in)
	else:
		_seq_batch = torch.cat((seqin.narrow(1, 0, _sel_sent), seqin.narrow(1, _sel_sent + 1, _nsent_in - _sel_sent),), dim=1)

	return clear_pad(_seq_batch), clear_pad(seqin.select(1, _sel_sent)), _sel_sent
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	for nsent, i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		with torch_autocast(enabled=_use_amp):
			output = model(seq_batch, oi, rsentind=sent_sel)
...
def eva(ed, nd, model, lossf, mv_device, multi_gpu, use_amp=False):
	...
	with torch_inference_mode():
		for nsent, i_d in tqdm(nd, mininterval=tqdm_mininterval):
			...
			r += correct.sum().item()
			correct = data_mask = trans = loss = output = ot = oi = seq_batch = seq_o = _src_mask = None
	w = float(w)
...
lrsch = InverseSqrtLR(optimizer, lr=init_lr)#GoogleLR(optimizer, cnfg.isize, cnfg.warm_step)
