	...
	with torch_inference_mode():
		for nsent, i_d in tqdm(nd, mininterval=tqdm_mininterval):
			...
			with torch_autocast(enabled=use_amp):
				...
				if multi_gpu:
					loss = loss.sum()
					trans = torch.cat(pad_tensors([outu.argmax(-1).to(mv_device, non_blocking=True) for outu in output], dim=2), 0)
				else:
					trans = output.argmax(-1)
			sum_loss += loss.data.item()
			ot = ot.narrow(2, 0, trans.size(2))
			data_mask = ot.ne(pad_id)
			correct = (trans.eq(ot) & data_mask).int()
			w += data_mask.int().sum().item()
			r += correct.sum().item()
			correct = data_mask = trans = loss = output = ot = oi = seq_batch = seq_o = None
	w = float(w)
...
if fine_tune_m is not None:
	...
	if cnfg.freeze_load_model:
		freeze_module(_tmpm)
		if cnfg.unfreeze_bias:
			_tmpm.dec.classifier.bias.requires_grad_(True)
		if fine_tune_reg_w is not None:
			_tmpm.dec.classifier.weight.requires_grad_(True)
	mymodel.load_base(_tmpm)
	_tmpm = None

if fine_tune_reg_w is not None:
	mymodel.dec.unbind_classifier_weight()
	base_para_group = backup_para_data(mymodel.get_loaded_train_paras())

lossf = ReducedDocLabelSmoothingLoss(nwordt, cnfg.label_smoothing, ignore_index=pad_id, reduction="sum", forbidden_index=cnfg.forbidden_indexes)
...
if cuda_device:
	...
	base_para_group = [_tu.to(cuda_device, non_blocking=True) for _tu in base_para_group]
