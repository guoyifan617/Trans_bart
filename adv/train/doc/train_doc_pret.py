...
from transformer.MDoc.NMT import NMT as MDocNMT
...
from utils.doc.pret.base4torch import patch_pret_model_ffn, split_doc
from utils.finetune import regularize_grad
...
import cnfg.docmonobase as mdocnfg
import cnfg.docpret as cnfg
from cnfg.ihyp import *
from cnfg.vocab.base import pad_id

def custom_optm_step(optm, model=None, scaler=None, closure=None, multi_gpu=False, multi_gpu_optimizer=False, zero_grad_none=optm_step_zero_grad_set_none):

	global fine_tune_reg_w, base_para_group

	if multi_gpu:
		model.collect_gradients()
	if fine_tune_reg_w is not None:
		regularize_grad(base_para_group, model.module.get_loaded_paras(), weight=fine_tune_reg_w)
	if scaler is None:
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	src_grp, pret_grp, tgt_grp = td["src"], td["pret"], td["tgt"]
	for nsent, i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		for sbu, oiu, otu, start_sent_id in split_doc(seq_batch, seq_o, max_sent=24):
			with torch_autocast(enabled=_use_amp):
				output = model(sbu, oiu, seq_pret, start_sent_id=start_sent_id)
				loss = lossf(output, otu)
				if multi_gpu:
					loss = loss.sum()
		...
		if _done_tokens >= tokens_optm:
			custom_optm_step(optm, model=model, scaler=scaler, multi_gpu=multi_gpu, multi_gpu_optimizer=multi_gpu_optimizer, zero_grad_none=optm_step_zero_grad_set_none)
			_done_tokens = 0
	...
	with torch_inference_mode():
		for nsent, i_d in tqdm(nd, mininterval=tqdm_mininterval):
			...
			with torch_autocast(enabled=use_amp):
				output = model(seq_batch, oi, seq_pret)
...
if fine_tune_m is not None:
	...
	if cnfg.fine_tune_mdoc is None:
		_tmpmdocenc = None
	else:
		logger.info("Load pre-trained MDoc from: " + cnfg.fine_tune_mdoc)
		_tmpmdoc = patch_pret_model_ffn(MDocNMT(mdocnfg.isize, nwordp, nwordp, mdocnfg.nlayer, mdocnfg.ff_hsize, mdocnfg.drop, mdocnfg.attn_drop, mdocnfg.share_emb, mdocnfg.nhead, cache_len_default, mdocnfg.attn_hsize, mdocnfg.norm_output, mdocnfg.bindDecoderEmb, mdocnfg.forbidden_indexes))
		_tmpmdoc = load_model_cpu(cnfg.fine_tune_mdoc, _tmpmdoc)
...
optimizer = Optimizer(filter_para_grad(mymodel.parameters()), lr=cnfg.fine_tune_lr, betas=adam_betas_default, eps=ieps_adam_default, weight_decay=cnfg.weight_decay, amsgrad=use_ams)
...
lrsch = None#GoogleLR(optimizer, cnfg.isize, cnfg.warm_step)
...
if fine_tune_m is None:
	...
	if cnt_states is not None:
		...
		if "training_list" in _remain_states:
			...
			shuffle(tl)
			_ctl = tl
		tminerr, done_tokens, cur_checkid, remain_steps, _ = train(td, _ctl, vd, vl, optimizer, lrsch, mymodel, lossf, cuda_device, logger, done_tokens, multi_gpu, tokens_optm, batch_report, save_every, chkpf, state_holder, statesf, num_checkpoint, cur_checkid, report_eva, remain_steps, False, False, scaler)
...
for i in range(1, maxrun + 1):
	shuffle(tl)
	free_cache(use_cuda)
	terr, done_tokens, cur_checkid, remain_steps, _Dws = train(td, tl, vd, vl, optimizer, lrsch, mymodel, lossf, cuda_device, logger, done_tokens, multi_gpu, tokens_optm, batch_report, save_every, chkpf, state_holder, statesf, num_checkpoint, cur_checkid, report_eva, remain_steps, dss_ws > 0, i >= start_chkp_save, scaler)
	vloss, vprec = eva(vd, vl, mymodel, lossf, cuda_device, multi_gpu, use_amp)
	...
	if (vprec <= minerr) or (vloss <= minloss):
		...
		if namin >= earlystop:
			if done_tokens > 0:
				custom_optm_step(optimizer, model=mymodel, scaler=scaler, multi_gpu=multi_gpu, multi_gpu_optimizer=multi_gpu_optimizer)
				done_tokens = 0
