...
from transformer.HPLSTM.FNLM import Decoder as NMT
...
def eva(ed, nd, model, lossf, mv_device, multi_gpu, use_amp=False):
	...
	sum_loss = 0.0
	model.eval()
	last_ind = ed.size(-1) - 1
	curind = 0
	_acc_len = 0
	states = {}
	global step_size, reset_size
	if multi_gpu:
	...
	with torch_inference_mode():
		while curind <= last_ind:
			...
			with torch_autocast(enabled=use_amp):
				output, states = model(seq_o.narrow(1, 0, lo), states=states)
				loss = lossf(output, ot)
...
data = torch.load(cnfg.train_data)

td_full = data["train"]
vd = data["test"]

ntrain = td_full.size(0) // (bsize * step_size)
nvalid = None
td = td.narrow(0, 0, bsize * step_size * ntrain).view(ntrain, bsize, step_size)

nword = td_full.max().item() + 1
nwordi, nwordt = nword, nword

tl = list(range(ntrain))

logger.info("Design models with seed: %d" % torch.initial_seed())
mymodel = NMT(cnfg.isize, nwordt, cnfg.nlayer, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, emb_w=None, num_head=cnfg.nhead, xseql=cache_len_default, ahsize=cnfg.attn_hsize, norm_output=cnfg.norm_output, bindemb=cnfg.bindDecoderEmb, forbidden_index=cnfg.forbidden_indexes)
...
lossf = NLLLoss(ignore_index=0, reduction="sum")
