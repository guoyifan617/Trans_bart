...
if len(sys.argv) == 4:
	...
	for modelf in sys.argv[3:]:
		tmp = NMT(cnfg.isize, nwordi, nwordt, cnfg.nlayer, cnfg.ff_hsize, cnfg.drop, cnfg.attn_drop, cnfg.share_emb, cnfg.nhead, cache_len_default, cnfg.attn_hsize, cnfg.norm_output, cnfg.bindDecoderEmb, cnfg.forbidden_indexes, nmimo=cnfg.nmimo)
...
with sys_open(sys.argv[1], "wb") as f, torch_inference_mode():
	for i in tqdm(range(ntest), mininterval=tqdm_mininterval):
		...
		with torch_autocast(enabled=use_amp):
			output = mymodel.decode(seq_batch, beam_size, None, length_penalty, ensemble_decoding=cnfg.ensemble_decoding)
		if multi_gpu:
			tmp = []
			for ou in output:
