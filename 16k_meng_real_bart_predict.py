...
import cnfg.plm.bart.base_16kmeng2zhtest as cnfg
from cnfg.plm.bart.ihyp import *
from cnfg.vocab.plm.bert_ch import eos_id, sos_id
from cnfg.vocab.plm.bert_ch import vocab_size as ch_vocab_size
...
detoken = Tokenizer(vocab_file=sys.argv[2]).decode
	...
	for i in tqdm(range(td["ndata"][()].item()), mininterval=tqdm_mininterval):
		...
		for tran in output:
			...
			for tmpu in tran:
				...
				if tmpu == eos_id:
					break
			f.write(detoken(tmp, skip_special_tokens=True, clean_up_tokenization_spaces=True).encode("utf-8"))
			f.write(ens)
